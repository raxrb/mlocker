...
//var x=sjcl.encrypt("This is going to be encryted");
//alert(sjcl.codec.hex.fromBits(x));
//"use strict";
function q(a) {
    throw a;
}
...
function y(a, b, c) {
    ...
    b = b[c ? 1 : 3] ^ d[3];
    var h, l, k, n = d.length / 4 - 2, m, p = 4, s = [0, 0, 0, 0];
    h = a.j[c];
    a = h[0];
...
sjcl.codec.base64 = {I: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", fromBits: function (a, b, c) {
        ...
        a = a.replace(/\s|=/g, "");
        var c = [], d, e = 0, f = sjcl.codec.base64.I, g = 0, h;
        b && (f = f.substr(0, 62) + "-_");
        for (d = 0; d < a.length; d++)
...
sjcl.mode.ccm = {name: "ccm", encrypt: function (a, b, c, d, e) {
        ...
        e = e || 64;
        d = d || [];
        var f = sjcl.bitArray, g = f.bitLength(c) / 8, h = f.bitLength(b), l = f.clamp(b, h - e), k = f.bitSlice(b,
                h - e), h = (h - e) / 8;
...
sjcl.mode.ocb2 = {name: "ocb2", encrypt: function (a, b, c, d, e, f) {
        ...
        var g, h = sjcl.mode.ocb2.G, l = sjcl.bitArray, k = l.k, n = [0, 0, 0, 0];
        c = h(a.encrypt(c));
        var m, p = [];
        d = d || [];
        e = e || 64;
        ...
        128 !== sjcl.bitArray.bitLength(c) && q(new sjcl.exception.invalid("ocb iv must be 128 bits"));
        e = e || 64;
        var g = sjcl.mode.ocb2.G, h = sjcl.bitArray, l = h.k, k = [0, 0, 0, 0], n = g(a.encrypt(c)), m, p, s = sjcl.bitArray.bitLength(b) - e, r = [];
        d = d || [];
...
sjcl.mode.gcm = {name: "gcm", encrypt: function (a, b, c, d, e) {
        ...
        b = b.slice(0);
        for (d = 0; d < e; d += 4)
            b[0] ^= 0xffffffff & c[d], b[1] ^= 0xffffffff & c[d + 1], b[2] ^= 0xffffffff & c[d + 2], b[3] ^= 0xffffffff & c[d + 3], b = sjcl.mode.gcm.U(b, a);
        return b
    }, n: function (a, b, c, d, e, f) {
...
sjcl.prng.prototype = {randomWords: function (a, b) {
        ...
        for (d = 0; d < a; d += 4)
            0 === (d + 1) % this.Q && A(this), e = B(this), c.push(e[0], e[1], e[2], e[3]);
        A(this);
        return c.slice(0, a)
...
sjcl.json = {
    defaults: {v: 1, iter: 1E3, ks: 128, ts: 64, mode: "ccm", adata: "", cipher: "aes"}, encrypt: function (a, b, c, d) {
        ...
        c = c || {};
        d = d || {};
        // password || key , plaintext , p ( tells about various modes and the other things ) ,rp contains details
        var e = sjcl.json, f = e.d({iv: sjcl.random.randomWords(4, 0)}, e.defaults), g;
        e.d(f, c);
        c = f.adata;
        ...
        "string" === typeof a && (g = sjcl.misc.cachedPbkdf2(a, f), a = g.key.slice(0, f.ks / 32), f.salt = g.salt) && console.log("salt is created in sjcl");
        ...
        console.log("Salt is going to be made in Hex");
        for ( x in f.salt )
            console.log(f.salt[x].toString(16));


        console.log("Key is going to be made in Hex");
        for ( x in f.key ){
            console.log("This is the x value "+x);
            console.log(f.key[x].toString(16));
        }
        return e.encode(f);
        
//      This is going to be encrypted, And be careful
        ...
        var b = {}, c, d;
        for (c = 0; c < a.length; c++)
            (d = a[c].match(/^(?:(["']?)([a-z][a-z0-9]*)\1):(?:(\d+)|"([a-z0-9+\/%*_.@=\-]*)")$/i)) || q(new sjcl.exception.invalid("json decode: this isn't json!")), b[d[2]] = d[3] ? parseInt(d[3], 10) : d[2].match(/^(ct|salt|iv)$/) ?
